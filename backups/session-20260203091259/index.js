...
function loadState() {
    ...
    // FAQ Assistant removed (feature disabled per request)
...
// Also attach direct listeners to help-create-ticket buttons (more robust than delegation in some browsers)
document.querySelectorAll('.help-create-ticket').forEach(btn => {
    btn.addEventListener('click', (e) => {
        ...
        if (subjEl) subjEl.value = subject;
        if (descEl) descEl.value = desc;
        console.debug('help-create-ticket clicked -> switching to new view', subject);
        switchView('new');
        setTimeout(() => { subjEl && subjEl.focus(); }, 80);
    });
});
...
/* --- FAQ Assistant (client-side 'IA' improved) --- */
// Normalize text (remove accents, punctuation) for robust matching
...
// Knowledge base: keywords (normalized) and answers
const FAQ_KB = [
    { keywords: ['internet','conexion','router','wifi'], answer: 'Prueba reiniciar el router/modem, desconectar y volver a conectar el Wi‑Fi, y si es general reinicia el equipo. Si sigue sin funcionar, crea un ticket con los pasos que has probado.' },
    { keywords: ['impresora','imprime','impresion','cola'], answer: 'Comprueba que la impresora esté encendida, papel/tóner y revisa la cola de impresión en el equipo. Reinicia la impresora y el PC. Si persiste, abre un ticket indicando modelo y mensajes de error.' },
    ...
    { keywords: ['contrasena','clave','olvide','restablecer'], answer: 'Usa la opción de restablecer contraseña del servicio. Si no existe, solicita el restablecimiento vía ticket indicando tu usuario y DNI.' },
    { keywords: ['aplicacion','app','error','se cierra','crash'], answer: 'Anota el mensaje de error y pasos para reproducirlo. Reinicia la app y, si persiste, adjunta capturas o logs en un ticket.' },
    ...
    { keywords: ['vpn','conexion vpn','vpn no conecta'], answer: 'Verifica credenciales y versión del cliente VPN, reinicia el cliente y la conexión. Si sigue fallando, adjunta logs del cliente en un ticket.' },
    { keywords: ['telefono','voip','fono','telefono'], answer: 'Reinicia el teléfono, revisa cables y configuración de red. Si hay caída general, crea un ticket indicando la extensión y síntoma.' }
].map(item => ({ kws: item.keywords.map(k => normalizeText(k)), answer: item.answer }));
...
function findAnswerFor(question){
    ...
    // score each KB entry by token overlap
    let best = null;
    let bestScore = 0;
    for(const item of FAQ_KB){
        let score = 0;
        for(const kw of item.kws){
            // if keyword is multiword, check substring, else token match
            if(kw.includes(' ')){
...
// Wire assistant UI safely (check elements exist)
const assistantInput = document.getElementById('assistant-input');
const assistantSend = document.getElementById('assistant-send');
const assistantSuggest = document.getElementById('assistant-suggest');
const assistantConvo = document.getElementById('assistant-convo');

// Welcome message when help view is opened
if(assistantConvo) appendAssistantBubble('Hola — soy el asistente. Pregúntame por problemas comunes (p.ej. "No puedo imprimir").', 'bot');
...
function assistantHandleSend(){
    ...
    setTimeout(()=>{
        // remove the 'analizando' last bot bubble if present
        const convo = document.getElementById('assistant-convo');
        if(convo){
...
// Create ticket from assistant suggestions (delegated)
document.addEventListener('click', (e)=>{
